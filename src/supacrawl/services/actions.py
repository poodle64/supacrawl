"""
Page actions service for Firecrawl-compatible page interactions.

Provides the ability to perform browser automation actions before scraping,
matching Firecrawl's actions API.
"""
...
@dataclass
class Action:
    """A single page action (Firecrawl-compatible).
    ...
    type: Literal[
        "wait", "click", "type", "write", "scroll", "screenshot", "press", "executeJavascript",
        "scrape"
    ]
...
def parse_actions(actions_json: list[dict[str, Any]]) -> list[Action]:
    ...
    Args:
        actions_json: List of action dictionaries (Firecrawl format).
