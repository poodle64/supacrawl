...
class ScrapeService:
    ...
    async def scrape(
        self,
        url: str,
        formats: list[Literal["markdown", "html", "rawHtml", "links", "screenshot", "pdf", "json", "images", "branding", "summary"]] | None = None,
        ...
        Args:
            ...
            max_age: Cache freshness in seconds (Firecrawl-compatible). 0 = no cache.
        ...
        try:
            ...
            try:
                ...
                if not self._stealth and _looks_like_bot_block(
                    page_content.status_code, page_content.html, markdown
                ):
                    if _is_patchright_available():
                        LOGGER.info(
                            f"Bot detection suspected for {url}, retrying with stealth mode"
                        )
                        ...
                        LOGGER.warning(
                            f"Bot detection suspected for {url}.{_stealth_hint()}"
                        )
                ...
                if captcha_detected:
                    if self._solve_captcha:
                        ...
                        LOGGER.warning(
                            f"CAPTCHA detected for {url}.{_captcha_hint()}"
                        )

                if "html" in formats:
                    # Clean HTML (boilerplate removed)
                    html = self._get_clean_html(
                        page_content.html, only_main_content,
            ...
            if not self._stealth and any(
                ...
                for pattern in ["403", "429", "timeout", "blocked", "denied"]
            ):
    ...
    async def _extract_json(
        ...
        class NoOpScrapeService:
            ...
            async def scrape(self, url: str, **kwargs):
                from supacrawl.models import ScrapeResult, ScrapeData, ScrapeMetadata
                return ScrapeResult(
                    success=True,
                    data=ScrapeData(  # type: ignore[call-arg]
                        markdown=markdown,
                    ...
                    if solved:
                        ...
                        return ScrapeResult(
                            ...
                            data=ScrapeData(  # type: ignore[call-arg]
                                ...
                                html=self._get_clean_html(html, only_main_content, include_tags, exclude_tags) if "html" in formats else None,
