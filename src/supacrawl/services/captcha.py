...
class CaptchaSolver:
    ...
    def _ensure_configured(self) -> None:
        ...
        if not _is_captcha_available():
            raise CaptchaNotInstalledError(
                "CAPTCHA solving requires the captcha extra. "
                "[HINT: Install with: pip install supacrawl[captcha]]"
            )

        if not self.api_key:
            raise CaptchaNotConfiguredError(
                "CAPTCHA API key not configured. "
    ...
    async def solve(self, captcha_info: CaptchaInfo) -> str:
        ...
        logger.info(
            f"Solving {captcha_info.captcha_type.value} CAPTCHA "
            f"(sitekey: {captcha_info.sitekey[:20]}...)"
        )
        ...
        try:
            ...
            if captcha_info.captcha_type == CaptchaType.RECAPTCHA_V2:
                ...
                raise CaptchaSolverError(
                    f"Unsupported CAPTCHA type: {captcha_info.captcha_type}"
                )
            ...
            if "ERROR_WRONG_USER_KEY" in error_msg or "ERROR_KEY_DOES_NOT_EXIST" in error_msg:
                raise CaptchaSolverError(
                    ...
                    "[HINT: Check your CAPTCHA_API_KEY environment variable is correct]"
                )
                ...
                raise CaptchaSolverError(
                    "No CAPTCHA solving slots available. "
                    "[HINT: Try again in a few seconds]"
                )
            else:
                raise CaptchaSolverError(f"CAPTCHA solving failed: {error_msg}")

    async def inject_solution(
        self, page: Page, captcha_info: CaptchaInfo, solution: str
    ) -> None:
