...
from supacrawl.discovery import (
    discover_sitemaps,
    fetch_robots,
...
async def _extract_browser_links(
    ...
    """
    Extract URLs from a page using browser rendering (Playwright).

    This is necessary for SPAs where links are JavaScript-rendered and not
    visible in raw HTML. Uses headless Chromium to render the page.

    Args:
        url: URL to render and extract links from.
        timeout_ms: Page load timeout in milliseconds.
        spa_delay: Extra delay for SPA content to stabilize.

    Returns:
        List of absolute URLs found in the rendered page.
    """
    links: list[str] = []

    try:
        headless = os.getenv("SUPACRAWL_HEADLESS", "true").lower() == "true"
        ...
        LOGGER.debug("Browser extracted %d links from %s", len(links), url)
        return links

    except Exception as e:
        LOGGER.warning("Failed to extract browser links from %s: %s", url, e)
        return []
...
async def map_site(
    ...
    Args:
        ...
        use_browser: If True, use Playwright to render pages for link extraction
            (required for SPAs with JavaScript-rendered links).
    ...
    # Use browser rendering for SPAs, otherwise use raw HTTP
    if not include_entrypoints_only and len(url_entries) < max_urls:
        link_source = "browser_links" if use_browser else "html_links"

        for entrypoint in config.entrypoints:
            if len(url_entries) >= max_urls:
                break

            # Choose extraction method based on use_browser flag
            if use_browser:
                LOGGER.info("Using browser to extract links from %s", entrypoint)
                links = await _extract_browser_links(entrypoint)
            else:
