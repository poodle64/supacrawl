...
- **`SEARXNG_USERNAME` / `SEARXNG_PASSWORD`**: discrete HTTP Basic credentials for a SearXNG instance behind an auth gate, so the instance URL stays a plain URL. Both optional and independent of availability — an ungated instance still needs only `SEARXNG_URL`, and half a credential is refused with a warning naming the missing variable rather than silently dropped.
